		...
		async function fetchData() {
			try {
				...
				if (msg['pump_id'] === pumpnumber) {
					...
					var countUp = new CountUp('PreviewPrice', RealTimePrice, {
						startVal: PreviewPrice,
						decimalPlaces: decimal_price,
						duration: 3
					});
					var countUp2 = new CountUp('PreviewUnit', RealTimePrice / PricePerUnit, {
						startVal: PreviewUnit,
						decimalPlaces: decimal_unit,
						duration: 3
					});
	...
	<div class="min-w-max min-h-max">
		...
		<div class="relative w-[1080px] h-[435px]">
			...
			<span class="absolute bottom-16 left-40 w-[105px] inline-block self-center">
				<h1
					class="text-center h-[118px] w-[103px] text-white text-[90px] antialiased font-sans p-0 font-medium leading-tight"
				>
					{$page.params.pumpId}
				</h1>
			</span>
